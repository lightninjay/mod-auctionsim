...
// functions (Suggest / SuggestAsync -- the latter used here, since this endpoint
// is open to every connected player with no rate limiting: a per-item drop-chance
// DB query has to go through AuctionSim::AddQueryCallback rather than block this
// hook, or a burst of requests could stall the whole server). If you're adding
// something here later, that read-only invariant is the whole point of this file
// being separate: keep it that way.
//
